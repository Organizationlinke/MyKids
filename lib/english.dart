...
// // const String geminiApiKey = 'AIzaSyBKcoraXz2NNtooOvPbZBbozIoOrFaRr7s'; 
...
//   void initState() {
//     super.initState();
//     // 0: كلمات, 1: ar(كامل), 2: L1, 3: L2, 4: L3, 5: L4, 6: L5(en)
//     _tabController = TabController(length: 7, vsync: this); 
//     fetchGroupData();
//   }
...
//         final storyIds = storiesData.map((s) => s['id']).toList();
//         levelsData = await supabase.from('w_story_level').select().inFilter('w_story_id', storyIds);
//         // ترتيب المستويات لضمان العرض الصحيح
...
//     final wordListEn = words.map((w) => w['word_en']).join(', ');
//     final wordListAr = words.map((w) => w['word_ar']).join(', ');
    
//     // برومبت ذكي لإنشاء كافة المستويات في طلب واحد بصيغة JSON
//     final prompt = """
//       Create a creative story based on these words: [$wordListEn].
//       Return ONLY a JSON object with this exact structure:
//       {
//         "title": "catchy title",
//         "story_en": "The full story in English",
//         "story_ar": "The full story in Arabic",
//         "levels": {
//           "1": "Arabic story where ONLY these specific words [$wordListEn] are in English.",
//           "2": "Arabic story where the specific words and 25% of other common words are in English.",
//           "3": "Arabic story where the specific words and 50% of other common words are in English.",
//           "4": "Arabic story where the specific words and 75% of other common words are in English.",
//           "5": "The full English story (same as story_en)."
//         }
//       }
//       Ensure the story is consistent across all levels.
//     """;
...
//       // 2. تجهيز مصفوفة المستويات لحفظها دفعة واحدة (Bulk Insert)
...
//       // 3. حفظ كل المستويات في طلب واحد لقاعدة البيانات
//       await supabase.from('w_story_level').insert(levelsToInsert);

//       await fetchGroupData();
//       _tabController.animateTo(2); // الانتقال لمستوى L1 مباشرة بعد الإنشاء
...
//               children: [
//                 _buildWordsTab(),
//                 _buildBaseArabicTab(), // التبويب العربي (القصة الأصلية)
//                 _buildStoryLevelTab(1),
//                 _buildStoryLevelTab(2),
//                 _buildStoryLevelTab(3),
//                 _buildStoryLevelTab(4),
...
//             icon: const Icon(Icons.auto_awesome),
//             label: const Text('إنشاء كافة مستويات القصة'),
//             style: ElevatedButton.styleFrom(
//               minimumSize: const Size.fromHeight(50),
//               backgroundColor: Colors.blue[800],
//               foregroundColor: Colors.white,
