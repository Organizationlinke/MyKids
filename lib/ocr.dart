...
class _OCRScannerScreenState extends State<OCRScannerScreen> {
  ...
  Future<void> _initializeCamera() async {
    ...
    // --- (التعديل) ---
    // البحث عن الكاميرا الخلفية تحديداً
    CameraDescription rearCamera;
    try {
      rearCamera = cameras.firstWhere(
        (camera) => camera.lensDirection == CameraLensDirection.back,
      );
    } catch (e) {
      // في حالة عدم العثور على كاميرا خلفية، استخدم الأولى (للاحتياط)
      print("لم يتم العثور على كاميرا خلفية، سيتم استخدام الكاميرا الافتراضية: $e");
      rearCamera = cameras.first;
    }
    // --- (نهاية التعديل) ---

    _cameraController = CameraController(
      rearCamera, // استخدام الكاميرا الخلفية التي تم إيجادها
      ResolutionPreset.high,
      enableAudio: false,
    );
  ...
  Future<void> _processImageStream(CameraImage image) async {
    ...
    try {
      ...
      // regex للبحث عن 3 أرقام أو أكثر متتالية
      final RegExp numberRegExp = RegExp(r'\b\d{3,}\b');
  ...
  Future<void> _saveNumberToFirestore() async {
    ...
    try {
      // استخدام مسار يتوافق مع قواعد الأمان
      final collectionPath = 'artifacts/${widget.userId}/scans';
      await FirebaseFirestore.instance.collection(collectionPath).add({
        'number': _recognizedNumber,
        'scan_date': FieldValue.serverTimestamp(),
        'user_id': widget.userId,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          CameraPreview(_cameraController!),
          if (_numberBoundingBox != null && _cameraController!.value.previewSize != null)
            CustomPaint(
              painter: BoxPainter(
                box: _numberBoundingBox!,
                // تبديل العرض والارتفاع ليناسب اتجاه الكاميرا
                imageSize: Size(
                  _cameraController!.value.previewSize!.height,
                  _cameraController!.value.previewSize!.width,
                ),
...
class BoxPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // دالة لضبط المقياس بين حجم صورة الكاميرا وحجم الشاشة
    Rect scaleRect(Rect rect, Size imageSize, Size widgetSize) {
      final double scaleX = widgetSize.width / imageSize.width;
      final double scaleY = widgetSize.height / imageSize.height;

      // استخدام `applyBoxFit` لإيجاد المقياس الصحيح (مثل `BoxFit.cover`)
      final FittedSizes fittedSizes =
          applyBoxFit(BoxFit.cover, imageSize, widgetSize);
      final Size sourceSize = fittedSizes.source;
      final Size destinationSize = fittedSizes.destination;

      final double scale = destinationSize.width / sourceSize.width;

      // حساب الإزاحة (offset) لمحاذاة المستطيل في المركز
      ...
      final double scaledLeft = rect.left * scale + offsetX;
      final double scaledTop = rect.top * scale + offsetY;
      final double scaledRight = rect.right * scale + offsetX;
      final double scaledBottom = rect.bottom * scale + offsetY;
